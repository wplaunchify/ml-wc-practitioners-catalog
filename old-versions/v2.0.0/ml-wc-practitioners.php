...
define('ML_WC_PRACTITIONERS_VERSION', '2.0.0');
...
define('ML_WC_PRACTITIONERS_GITHUB_REPO', 'wplaunchify/ml-wc-practitioners-catalogue');
define('ML_WC_PRACTITIONERS_GITHUB_BRANCH', 'master');
...
class ML_WC_Practitioners_Catalog {
    ...
    /**
     * Default GitHub token (must be set in plugin settings)
     * This repo is PUBLIC so no token needed for image downloads
     */
    ...
    public function render_admin_page() {
        ...
        <div class="wrap ml-practitioners-wrap">
            ...
            <div class="ml-practitioners-card">
                ...
                <div class="ml-practitioners-form-group">
                    <label for="ml-practitioners-github-token">GitHub Personal Access Token</label>
                    <input 
                        type="password" 
                        id="ml-practitioners-github-token" 
                        value="<?php echo esc_attr($github_token); ?>"
                        placeholder="ghp_xxxxxxxxxxxxxxxxxxxx"
                    >
                    <p class="description">
                        Token for accessing the private ml-stuarthoover repository. 
                        Repository: <code><?php echo ML_WC_PRACTITIONERS_GITHUB_REPO; ?></code>
                    </p>
                </div>
                
                <div style="margin-top: 20px;">
                    <button type="button" id="ml-practitioners-save-settings" class="ml-practitioners-button">
                        Save Settings
                    </button>
                    <button type="button" id="ml-practitioners-test-connection" class="ml-practitioners-button secondary">
                        Test Connection
                    </button>
                </div>
            </div>
 ...
 */
    ...
    private function download_image_from_github($image_url) {
        $token = $this->get_github_token();
        $this->log('Original image URL: ' . $image_url);
        
        // Convert raw GitHub URL to API URL if needed
        if (strpos($image_url, 'raw.githubusercontent.com') !== false) {
            // Convert: https://raw.githubusercontent.com/user/repo/branch/path/file.png
            // To: https://api.github.com/repos/user/repo/contents/path/file.png
            $image_url = str_replace(
                'https://raw.githubusercontent.com/' . ML_WC_PRACTITIONERS_GITHUB_REPO . '/' . ML_WC_PRACTITIONERS_GITHUB_BRANCH . '/',
                'https://api.github.com/repos/' . ML_WC_PRACTITIONERS_GITHUB_REPO . '/contents/',
                $image_url
            );
            $this->log('Converted to API URL: ' . $image_url);
        }
